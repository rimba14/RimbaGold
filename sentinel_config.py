...
BASE_WATCHLIST = [
    ...
    # High-Volatility US Equities (7)
    "SNOW", "DDOG", "NET", "AMD", "RIOT", "MARA", "UPST"
]
...
# ---------------------------------------------------------------------------
# HARDWARE FABRIC INTEGRATION (v32.0-PROD)
# ---------------------------------------------------------------------------
import platform
_SYS_PLATFORM = platform.system()
PHOTONIC_FABRIC_PATH = 'C:/dev/photonic_fabric/shared_pool0' if _SYS_PLATFORM == 'Windows' else '/dev/photonic_fabric/shared_pool0'
PHOTONIC_FABRIC_ACTIVE = os.path.exists(PHOTONIC_FABRIC_PATH)
if PHOTONIC_FABRIC_ACTIVE:
    logging.info(f'[HARDWARE] Photonic Fabric Detected at {PHOTONIC_FABRIC_PATH}. Initializing in-memory HBM pointers.')
    ARCTIC_URI = f'memory://'
else:
