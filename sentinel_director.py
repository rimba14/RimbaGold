...
def run_regression_lockbox():
    print("[DIRECTOR] Boot Sequence: Executing CADES Regression Lockbox...")
    test_script = os.path.join(os.path.dirname(__file__), "tests", "regression", "test_cades_lockbox.py")
    try:
        result = subprocess.run([sys.executable, "-m", "pytest", test_script], capture_output=True, text=True)
        if result.returncode != 0:
            print(result.stdout)
            raise Exception("[CRITICAL_VERSION_DRIFT] Regression Lockbox failed! Halting deployment to prevent live capital risk.")
        print("[DIRECTOR] CADES Regression Lockbox passed. Absolute Guardrails confirmed.")
    except Exception as e:
        if "[CRITICAL_VERSION_DRIFT]" in str(e):
            raise
        raise Exception(f"[CRITICAL_VERSION_DRIFT] Failed to execute Regression Lockbox: {e}")

async def main():
    print("[DIRECTOR] Starting Sentinel Central Director Orchestrator loop...")
    run_regression_lockbox()
