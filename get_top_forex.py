...
non_forex_keys = {"BTC", "ETH", "SOL", "AVAX", "LINK", "LTC", "BCH", "XRP", "ADA", "DOT", "MATIC", "DOGE", "UNI", "ATOM", "TRX", "XAU", "XAG", "US30", "NAS100", "SPX", "US2000", "GER40", "HK50", "GOLD", "SILVER", "OIL"}
...
for sym in lib.list_symbols():
    if sym.endswith("_meta"):
        ...
        if any(k in base_sym.upper() for k in non_forex_keys):
            continue
        ...
        if not data.empty:
            ...
            if conviction >= 0.5: # Allow low conviction to fill top 5, but we sort by it anyway
                assets.append({
                    "symbol": base_sym,
                    "direction": direction,
