...
class QwenReasoningEngine:
    ...
    def json_with_retry(
        ...
        for attempt in range(max_retries):
            ...
            # Phase 4: Reasoning Isolation (<think> tag parsing)
            if "<think>" in raw and "</think>" in raw:
                raw_json_part = raw.split("</think>")[-1].strip()
            else:
                raw_json_part = raw
                
            try:
                start = raw_json_part.find("{")
                end   = raw_json_part.rfind("}") + 1
                if start != -1 and end > start:
                    data = json.loads(raw_json_part[start:end])
