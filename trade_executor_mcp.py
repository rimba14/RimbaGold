...
def execute_trade(symbol, conviction, hmm_regime):
    ...
    try:
        # Phase 4: Deterministic Fallbacks
        # Fallback to yfinance if MT5 returns TICKER_ERROR or fails
        info = mt5.symbol_info(symbol)
        yf_price_fallback = None
        
        if not info:
            import yfinance as yf
            logging.warning(f"[FALLBACK] MT5 symbol_info failed for {symbol}. Failing over to yfinance 1-minute aggregations.")
            try:
                yf_sym = symbol
                if "USD" in symbol and len(symbol) == 6: yf_sym = f"{symbol[:3]}=X"
                elif symbol == "BTCUSD": yf_sym = "BTC-USD"
                elif symbol == "ETHUSD": yf_sym = "ETH-USD"
                elif symbol == "SP500": yf_sym = "^GSPC"
                
                df = yf.Ticker(yf_sym).history(period="1d", interval="1m")
                if not df.empty:
                    yf_price_fallback = float(df['Close'].iloc[-1])
            except Exception as e:
                logging.error(f"[FALLBACK] yfinance also failed: {e}")
                
            if yf_price_fallback is None:
                return {"status": "error", "message": f"SYMBOL_NOT_FOUND_AND_FALLBACK_FAILED: {symbol}"}
        else:
            if not info.visible:
        ...
        price_proxy = tick.bid if tick else (yf_price_fallback if yf_price_fallback else 0.0)
        ...
        if tick:
            ...
            price = yf_price_fallback if yf_price_fallback else 0.0
