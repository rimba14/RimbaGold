...
def main():
    ...
    for pos in all_pos:
        ...
        # Directive v38: Edge Validation (Walk-Forward Bootstrap CI)
        # Evaluate rolling IC over 60-days. Cull dynamic rules if < 0.05
        parameter_active = True
        try:
            import os, json
            ic_state_path = r"C:\Sentinel_Project\data\alpha_ic_state.json"
            rolling_ic = 0.06
            if os.path.exists(ic_state_path):
                with open(ic_state_path, "r") as f:
                    ic_state = json.load(f)
                rolling_ic = ic_state.get(symbol, {}).get("dynamic_rules_ic", 0.06)
            
            if rolling_ic < 0.05:
                print(f"[CADES_EDGE_DECAY] {symbol} parameter IC ({rolling_ic}) < 0.05. Culling dynamic rules -> standard ATR floor.")
                parameter_active = False
        except Exception:
            pass
        
        # In range regimes, take profit is squashed
        if ("RANGE" in wasserstein_state or "RANGE" in hmm_state) and parameter_active:
        ...
        if parameter_active and ("RANGE" in wasserstein_state or "RANGE" in hmm_state):
